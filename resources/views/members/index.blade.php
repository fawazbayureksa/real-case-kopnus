...
<div class="row">
    <div class="col-12 d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-primary d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#createMemberModal">
                <i class="bi bi-plus-lg me-2"></i> Tambah Anggota
            </button>
            <button type="button" class="btn btn-outline-primary d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#uploadMemberModal">
...
<div class="row">
    <div class="col-12">
        ...
        @if($recentUpload)
        <div class="card border-0 shadow-sm mb-4" style="border-left: 5px solid {{ $recentUpload->status == 'completed' ? '#28a745' : ($recentUpload->status == 'processing' ? '#ffc107' : '#dc3545') }};">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h6 class="fw-bold mb-1">Status Upload Terakhir: <span class="badge {{ $recentUpload->status == 'completed' ? 'bg-success' : ($recentUpload->status == 'processing' ? 'bg-warning text-dark' : 'bg-danger') }}">{{ strtoupper($recentUpload->status) }}</span></h6>
                        <p class="text-muted small mb-0">File: {{ $recentUpload->filename }} | {{ $recentUpload->updated_at->diffForHumans() }}</p>
                    </div>
                    @if($recentUpload->status == 'completed' || $recentUpload->status == 'failed')
                    <div class="text-end">
                        <div class="d-flex gap-4">
                            <div class="text-center">
                                <span class="d-block h5 mb-0 fw-bold">{{ $recentUpload->total_success + $recentUpload->total_failed }}</span>
                                <span class="small text-muted">Total Data</span>
                            </div>
                            <div class="text-center">
                                <span class="d-block h5 mb-0 fw-bold text-success">{{ $recentUpload->total_success }}</span>
                                <span class="small text-muted">Berhasil</span>
                            </div>
                            <div class="text-center">
                                <span class="d-block h5 mb-0 fw-bold text-danger">{{ $recentUpload->total_failed }}</span>
                                <span class="small text-muted">Gagal</span>
                            </div>
                        </div>
                        @if($recentUpload->total_failed > 0)
                        <a href="{{ route('members.export-errors', $recentUpload->id) }}" class="btn btn-sm btn-outline-danger rounded-pill mt-2 fw-bold">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        <button class="btn btn-sm btn-outline-secondary px-3 rounded-pill">Detail</button>
                                        @can('manage members')
                                        <button class="btn btn-sm btn-outline-primary px-3 rounded-pill">Ubah</button>
                                        <button class="btn btn-sm btn-outline-danger px-3 rounded-pill">Hapus</button>
                                        @endcan
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="text-muted">
                                        <i class="bi bi-people mb-3 d-block" style="font-size: 3rem; opacity: 0.3;"></i>
            ...
            <form action="{{ route('members.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body py-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" class="form-control rounded-pill" id="file" name="file" accept=".csv,.xlsx,.xls" required>
                    </div>
                </div>
            ...
            <form action="{{ route('members.store') }}" method="POST">
                ...
                <div class="modal-body py-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control rounded-pill" id="member_number" name="member_number" placeholder="Contoh: KOP-2024-001" required>
                    </div>
                    <div class="mb-3">
                        <label for="name" class="info-label">Nama Lengkap</label>
                        <input type="text" class="form-control rounded-pill" id="name" name="name" placeholder="Masukkan nama lengkap" required>
                    </div>
